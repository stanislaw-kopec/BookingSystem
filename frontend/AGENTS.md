...
- Frontend ma jeden język interfejsu: polski. Nie dodawaj mechanizmu wielojęzyczności,
  słowników językowych `pl/en` ani przełącznika języka bez nowej decyzji użytkownika.
  Dane przychodzące z API, np. nazwy usług zapisane w bazie, pozostają danymi
  biznesowymi wyświetlanymi tak, jak zwróci je backend.
...
- Statusy przedstawiaj po polsku: `PENDING` to „Oczekujące”, `TIME_PROPOSED`
  to „Zaproponowano nowy dzień”, `CONFIRMED` to „Potwierdzone”,
  `READY_FOR_PICKUP` to „Czeka na odbiór”, `COMPLETED` to „Zakończone”,
  `CANCELLED` to „Odwołane”, a `REJECTED` to „Odrzucone”. Pokazuj pierwotny
  i proponowany dzień bez sugerowania wykonanej naprawy.
...
- Funkcja wizyt znajduje się w `features/appointments`: `appointmentsApi` sprawdza
  ...
  `StaffAppointmentsPage` i `StaffSchedulePage` chroni `RequireStaff`, a `AdminScheduleSettingsPage` i `AdminStaffAccountsPage` chroni `RequireAdmin`. Przewodnik znajduje się w
