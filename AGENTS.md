...
- Po angielsku nazywaj foldery, pliki, klasy, funkcje, zmienne, trasy frontendu,
  ...
  Objaśnienia dla użytkownika mają być po polsku. Interfejs aplikacji pozostaje
  po polsku i nie używa mechanizmu wielojęzyczności ani przełącznika języka. Zachowuj kod
...
| R04 | Pozycja „Moje pojazdy” w menu konta prowadzi do `/vehicles`. Klient może dodać i edytować pojazd z marką, modelem, rokiem produkcji, numerem rejestracyjnym i opcjonalnym VIN oraz przeglądać wyłącznie własne pojazdy. Wybranie pojazdu otwiera osobną podstronę `/vehicles/:vehicleId` ze szczegółami, edycją i historią napraw. |
...
| R12 | Klient może pobrać prostą fakturę PDF z historii napraw własnego pojazdu przy zakończonym zgłoszeniu `COMPLETED`, a mechanik lub administrator może pobrać tę samą fakturę z panelu personelu, aby wydrukować ją klientowi przy odbiorze auta. Faktura zawiera logo Mietek Customs, numer, datę wystawienia i sprzedaży, dane warsztatu, dane nabywcy imienne albo firmowe z profilu, pojazd, opis wykonanych prac, wyszczególnione pozycje robocizny i części, wartości netto i brutto oraz informację o płatności przy odbiorze. Dokument jest utrwalany w tabeli invoice_documents przy odbiorze auta: numer, daty, dane stron i pojazdu, pozycje, kwoty oraz gotowy PDF. Pobranie nie korzysta z bieżącego profilu. Starsze zakończone naprawy bez dokumentu są utrwalane przy pierwszym pobraniu na podstawie dostępnych wtedy danych. Kwoty netto i VAT są liczone per pozycja, a podsumowania są sumami wierszy. Korekty, numeracja księgowa i deklaracja zgodności prawno-księgowej pozostają poza zakresem. |
...
- Zakres pełnego fakturowania: korekty,
  docelowa numeracja księgowa, eksport dokumentów i ewentualna integracja księgowa.
  Obecna wersja utrwala dokument i PDF dla zakończonego zgłoszenia, bez płatności online
...
- Zrealizowane etapy: katalog usług, rejestracja, profil klienta, jego pojazdy,
  ...
  7A–8 audytu backendu: wspólny zegar, mapery DTO, wydzielona walidacja naprawy,
  podział obsługi wizyt według przypadków użycia, ochrona przejść statusów w encji,
  uporządkowane pakiety i testy integracyjne modułu `appointment`, wspólne reguły
  kont i danych pojazdu oraz ujednolicone wyjątki API.
